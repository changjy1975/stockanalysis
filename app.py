...
# --- 1. 網頁設定 ---
st.set_page_config(page_title="量化交易看板 PRO (趨勢轉折賣出版)", layout="wide")
...
def load_and_process_data(symbol, start, end):
    try:
        ...
        # --- 買賣訊號邏輯 (依據最新指示：趨勢死亡交叉或 RSI 過熱) ---
        # 買進：KD 金叉 + K < 30 + 站在月線 EMA20 之上
        df['Buy_Signal'] = (df['K'] > df['D']) & (df['K'].shift(1) <= df['D'].shift(1)) & \
                           (df['K'] < 30) & (df['Close'] > df['EMA20'])
        
        # 賣出：EMA10 跌破 EMA20 (死亡交叉) "或" RSI > 90 (極度超買)
        df['Sell_Signal'] = (df['EMA10'] < df['EMA20']) | (df['RSI'] > 90)
...
if df is not None:
    ...
    st.title(f"📈 {ticker_input} 技術指標看板 (死亡交叉或 RSI>90)")
    ...
    # 賣出標記 (紅色三角形) - 使用 Shift 避免在趨勢走弱期間出現過多重複訊號
    ...
    fig.add_hline(y=90, line_dash="dash", line_color="red", opacity=0.8, row=5, col=1, annotation_text="極端過熱 (90)")
    ...
    with c2:
        st.error("**🔴 賣出：趨勢反轉或極端過熱**\n1. EMA10 跌破 EMA20 (趨勢轉空)\n2. **或** RSI > 90 (噴發趕頂)")

else:
    st.error("數據載入失敗，請確認代碼與日期。")
